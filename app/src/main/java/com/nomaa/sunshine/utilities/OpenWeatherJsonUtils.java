...
//        long normalizedUtcStartDay = SunshineDateUtils.normalizeDate(now);
        ...
        for (int i = 0; i < jsonWeatherArray.length(); i++) {
            ...
            JSONObject dayNumber = jsonWeatherArray.getJSONObject(i);

            JSONObject dayForecast = dayNumber.getJSONObject("main");

            /*
             * We ignore all the datetime values embedded in the JSON and assume that
             * the values are returned in-order by day (which is not guaranteed to be correct).
             */
            dateTimeMillis = normalizedUtcStartDay + com.nomaa.sunshine.utilities.SunshineDateUtils.DAY_IN_MILLIS * i;

            pressure = dayForecast.getDouble(OWM_PRESSURE);
            humidity = dayForecast.getInt(OWM_HUMIDITY);
            high = dayForecast.getDouble(OWM_MAX);
            Log.v("high:", "" + high);
            high += (rand.nextInt(Integer.SIZE - 1) % 2) + 1;
            Log.v("new high:", "" + high);

            low = dayForecast.getDouble(OWM_MIN) - (rand.nextInt(Integer.SIZE - 1) % 2) - 1;

            JSONObject windObject = dayNumber.getJSONObject("wind");
            ...
            JSONObject weatherObject =
                    dayNumber.getJSONArray(OWM_WEATHER).getJSONObject(0);
